...
We would like to ensure that the coffee maker moves through the expected modes
of operation. We want to make sure that the coffee maker always transitions
through the following sequence of states:
  ...
  With Error:
    If an error occurs in any of the states above then the Coffee machine stays
    in the error state until it is reset and after which it returns to the
    Warmup state.
*/
