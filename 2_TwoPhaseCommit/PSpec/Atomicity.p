...
We would like to assert the atomicity property that:
  if a transaction is committed by the coordinator then it was agreed on by all
  participants
*/
spec AtomicityInvariant
observes eWriteTransResp, ePrepareResp, eMonitor_AtomicityInitialize {
  ...
  state WaitForEvents {
    ...
    on eWriteTransResp do (resp: tWriteTransResp) {
      ...
      if (resp.status == SUCCESS){
        ...
        format("participants sent success: {0}, participants sent error: {1}",
          participantsResponse[resp.transId][SUCCESS], participantsResponse[resp.transId][ERROR]
        );
      } else if (resp.status == ERROR) {
        assert participantsResponse[resp.transId][ERROR] > 0,
        format("Write transaction was responded as failed before receiving error from at least one participant") +
        format("participants sent success: {0}, participants sent error: {1}",
          participantsResponse[resp.transId][SUCCESS], participantsResponse[resp.transId][ERROR]
        );
      }
...
Note. the use of hot and cold states.
*/
spec Progress
observes eWriteTransReq, eWriteTransResp {
  var pendingTransactions: int;
  start state Init {
    on eWriteTransReq goto WaitForResponses
      with {pendingTransactions = pendingTransactions + 1;}
  }

  hot state WaitForResponses {
    on eWriteTransResp do {
      pendingTransactions = pendingTransactions - 1;
      if (pendingTransactions == 0) goto AllTransactionsFinished;
    }
